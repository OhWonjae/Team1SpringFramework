...
public class AuthController {
	 ...
	 * // 회원가입 post
	 * 
	 * @RequestMapping(value = "/joinForm", method = RequestMethod.POST) public
	 * String join(User user) throws Exception { logger.info("post join");
	 * 
	 * int result = usersService.idCheck(user); System.out.println(result); try { if
	 * (result == 1) { logger.info("join");
	 * 
	 * return "/user/joinForm"; } else if (result == 0) { usersService.join(user); }
	 * } catch (Exception e) { throw new RuntimeException(); } return "redirect:/";
	 * }
	 */
	 ...
	 * user_phone, Model model) throws Exception { User user =
	 * usersService.getUserid(user_name, user_phone); model.addAttribute("user",
	 * user); System.out.println(user.getUser_id()); return "/user/searchId"; }
	 */
	...
	public String updatePw(String user_password, Authentication auth) {
		usersService.getUser(auth.getName());
		BCryptPasswordEncoder bpe = new BCryptPasswordEncoder();
